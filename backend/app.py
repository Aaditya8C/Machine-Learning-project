    ...
    try:
        if name:
            ...
            if not isinstance(vector, list) or len(vector) != len(rec.features):
                return jsonify({'error': f"'vector' must be a list of {len(rec.features)} numbers."}), 400
            fv = np.array(vector)
            recs = rec.recs_essentials(vector=fv)
        else:
            return jsonify({'error': 'Either "name" or "vector" must be provided.'}), 400
...
def makeup():
    ...
    if not skin_tone or not skin_type:
        return jsonify({'error': '"skin_tone" and "skin_type" are required fields.'}), 400
    
    # Call the recommendation function for makeup
    makeup_recs = rec.makeup_recommendation(skin_tone, skin_type)
    
    if not makeup_recs:
        return jsonify({'message': 'No makeup products found for the given skin tone and type.'}), 404
